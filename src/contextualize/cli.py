...
def _should_confirm_login(provider_label: str) -> bool:
    is_tty = False
    try:
        is_tty = sys.stdin.isatty() and sys.stdout.isatty()
    except Exception:
...
def _print_arena_auth_status() -> None:
    from .cache.arena import get_cached_user_access_token, get_cached_user_token_record
...
def _run_arena_logout() -> None:
    from .cache.arena import clear_cached_user_token
...
def _store_arena_pat(token: str, *, profile: dict[str, str] | None = None) -> None:
    from .cache.arena import store_user_token
...
def auth_arena(
    ...
    """Show Are.na auth status, set token, or logout."""
    if logout_requested:
        _run_arena_logout()
        return
...
def _atproto_session_profile(
    ...
    *,
    ...
    if (
        ...
        from .references.atproto_auth import AtprotoDPoPAuth
...
def _atproto_has_active_user_auth() -> bool:
    from .cache.atproto import get_cached_oauth_access_token
...
def _atproto_refresh_cached_oauth_session() -> dict[str, object] | None:
    try:
        from .references.atproto import _refresh_cached_oauth_session
...
def _run_atproto_logout() -> None:
    from .cache.atproto import clear_cached_oauth_session
...
def _run_atproto_login(timeout: int, no_browser: bool) -> None:
    ...
    from .references.atproto_auth import (
        build_authorize_url,
        build_pkce_challenge,
        exchange_authorization_code,
...
def auth_atproto(logout_requested: bool, timeout: int, no_browser: bool) -> None:
    """Show ATProto auth status, login, or logout."""
    if logout_requested:
        _run_atproto_logout()
        return
    ...
    if not _should_confirm_login("ATProto"):
